...
#    Updated: 2021/06/19 00:26:49 by telron           ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
class PyChecker:
    ...
    def pack_moves(self):
        self.Moves = Frame(self.upframe, bg="gray")
        for line in [[["DEL", 0], ["PA", 0], ["PB", 0], ["SA", 0], ["SB", 0], ["SS", 0]], [["RA", 0], ["RB", 0], ["RR", 0], ["RRA", 0], ["RRB", 0], ["RRR", 0]]]:
            line_element = Frame(self.Moves, bg="gray")
            for name, cmd in line:
                button = Button(line_element, text=name, height=2, width=3)
                button.pack(side=LEFT)
            line_element.pack()
        self.Moves.pack()

    def pack_info(self):
        self.info = Frame(self.upframe, bg="gray")
        self.Move = Label(self.info, text='move = 0')
        self.Move.pack()
        self.Count = Label(self.info, text='Count move = ' + str(self.length_moves))
        self.Count.pack()
        self.info.pack()
